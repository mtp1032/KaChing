--[[
SellItems.lua — queued selling + robust tooltip + per-run pause (TWOW / Lua 5.0)
...
-- ===========================
-- Static popup for per-run pause (worded for buyback UX)
-- ===========================
...
-- Queue / state machine
-- ===========================
sell._queue   = sell._queue or {}   -- jobs: {bag,slot,name,count,tries,t0,state}
sell._current = nil
...
-- Confirm only by “slot became empty” (works even if buyback is at 12 constantly)
...
local function startRunner()
    ...
    sell._runner:SetScript("OnUpdate", function()
        ...
        -- Issue the sale then wait for confirmation / timeout
        ...
        if job.state == "waiting" then
            if saleConfirmed(job) then
                ...
                -- Per-run cap of 12, then show popup and pause
...
function sell.createKaChingButton()
    ...
    -- Keep tooltip above skins and hide when frame hides
    button:SetFrameStrata("HIGH")
    button:SetScript("OnHide", function() if GameTooltip then GameTooltip:Hide() end end)
    ...
    -- Tooltip (Classic: use 'this', with a name fallback just in case)
    button:EnableMouse(true)
    button:SetScript("OnEnter", function()
        local owner = this or getglobal("KaChingBtn")
...
if DEFAULT_CHAT_FRAME and core and core.debuggingIsEnabled and core:debuggingIsEnabled() then
    DEFAULT_CHAT_FRAME:AddMessage("SellItems.lua (tooltip+per-run pause) is loaded", 1, 1, 0.5)
end
