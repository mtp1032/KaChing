...
UPDATED: 10 August, 2025 (no buyback pre-check; per-run cap only)
]]
...
local safe  = KaChing.Safe    -- wrappers

-- ===========================
-- Static popup for buyback full (used as “batch pause” message)
-- ===========================
...
local function formatMoney(copper)
    ...
    if s > 0 or g > 0 then out = out .. "s " end
    return out .. c .. "c"
end
                        ...
                        if not KaChing.ExclusionList[nameLower] then
                            if isGrayByTooltip(bag, slot) or isGrayItemByLink(link) then
                                tinsert(sell._queue, {
                                    ...
                                    tries = 0, t0 = 0, buyback0 = 0, state = "pending"
                                })
                            end
                        end
                    end
...
local function startRunner()
    ...
    sell._runner:SetScript("OnUpdate", function()
        ...
        -- IMPORTANT: No pre-check of buyback count here anymore.
...
function sell.createKaChingButton()
    ...
    -- Tooltip (Lua 5.0 / Classic uses 'this' instead of 'self')
    button:EnableMouse(true)
    button:SetScript("OnEnter", function()
        if GameTooltip and this then
            GameTooltip:Hide() -- clear any prior owner
...
if DEFAULT_CHAT_FRAME and core and core.debuggingIsEnabled and core:debuggingIsEnabled() then
    DEFAULT_CHAT_FRAME:AddMessage("SellItems.lua (no-precheck build) is loaded", 1, 1, 0.5)
end
